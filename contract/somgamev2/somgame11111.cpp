...
ACTION somgame11111::resetwallet (name username) {
  require_auth(_self);
...
ACTION somgame11111::create (name contract, symbol symbol) {
  ...
  stats_table.emplace(_self, [&](somgame11111::stat &row) {
...
ACTION somgame11111::transfer (
  ...
  if (isWithdraw) {
    action(
      ...
      stat->contract,
      "transfer"_n,
      std::make_tuple(_self, to, quantity, std::string(memo))
    ).send();
...
void somgame11111::sub_balance (players::const_iterator player, asset asset) {
  players_table.modify(player, _self, [&](auto &row) {
    for (int i = 0; i < row.wallet.size(); i += 1) {
...
void somgame11111::add_balance (players::const_iterator player, asset asset) {
  players_table.modify(player, _self, [&](auto &row) {
    for (int i = 0; i < row.wallet.size(); i += 1) {
      if (row.wallet[i].symbol == asset.symbol) {
        row.wallet[i] += asset;
