<script lang="ts">
  ...
  import {ButtonComponent, ItemComponent, ModalComponent, TextComponent} from "ui";
...
<style>
  ...
  .flip-card-front {
    ...
    animation: border-animation 5s linear infinite alternate;
  }
...
<ModalComponent isClosable={false} dark={true}>
  <svelte:fragment slot="title">Chest unlocked</svelte:fragment>

  <svelte:fragment slot="info">
    {#if revealed}
      Hooray, you received
      <TextComponent color="{rarityNames.get(item.rarity)}">
        {rarityNames.get(item.rarity)} {item.name}
      </TextComponent>
      😊
    {:else}
      Click to reveal...
  ...
  <svelte:fragment slot="content">
    <div class="reveal">
      ...
      <div>
        ...
        {:else}
          <ButtonComponent on:click={onReveal}>REVEAL</ButtonComponent>
        {/if}
      </div>
    </div>
  </svelte:fragment>
