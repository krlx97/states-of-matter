<script lang="ts">
  ...
  const formStore = formService.create({
    address: ["", "name"],
    amount: ["", "item", item.shards],
  });

  const receipt = {
    owned: item.shards,
    remaining: item.shards
  };
  ...
  onMount((): void => {
    socketService.socket.on("getAddress" as any, async (params: any): Promise<void> => {
      ...
      const isConfirmed = await ethersService.transact(
        "collectibles",
        "safeTransferFrom",
        [$playerStore.address, params.address, item.id * 10n, $formStore.fields.amount.value, "0x"]
      );
...
<ModalComponent>
  ...
  <svelte:fragment slot="info">
    Transfer item shards to another player
  </svelte:fragment>
